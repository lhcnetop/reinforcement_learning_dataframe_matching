...
class DisturbedDataframe():
    ...
    def suffix_string(self, field:str, suffix_string, probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: x+suffix_string if rand.random()<=probability else x )
        )

    def prefix_string(self, field:str, prefix_string, probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: prefix_string+x if rand.random()<=probability else x )
        )

    def suffix_random_string(self, field:str, random_string_length:int, probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: x+get_random_string(random_string_length) if rand.random()<=probability else x )
        )

    def prefix_random_string(self, field:str, random_string_length:int, probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: get_random_string(random_string_length)+x if rand.random()<=probability else x ) 
        )


    def trim_left(self,field:str,length:int,probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: x[length:] if rand.random()<=probability else x )
        )
    ...
    def trim_char_left(self,field:str,chars:str,probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: x.lstrip(chars) if rand.random()<=probability else x )
        )

    def trim_char_right(self,field:str,chars:str,probability:float=1.0):
        self.df=self.df.with_columns(
            pl.col(field).apply(lambda x: x.rstrip(chars) if rand.random()<=probability else x )
        )